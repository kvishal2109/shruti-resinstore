...
import { Search, Sparkles, Home as HomeIcon, Sofa, Heart, Gem, ArrowRight, MapPin, MessageCircle, Youtube, Instagram, Facebook, ChevronLeft, ChevronRight } from "lucide-react";
    ...
    <div className="min-h-screen">
      ...
      <section className="relative py-16 px-4 overflow-hidden">
        ...
        <div className="container mx-auto max-w-7xl relative z-10">
          ...
          {/* Subcategories Panel - Shows when category is selected */}
          {selectedCategory && subcategories[selectedCategory as keyof typeof subcategories] && (
            <div className="mt-12 bg-white/95 backdrop-blur-md rounded-3xl shadow-2xl p-8 border-4 border-purple-300/50">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {subcategories[selectedCategory as keyof typeof subcategories].map((subcategory) => {
                  const categorySlug = categorySlugs[selectedCategory] || selectedCategory.toLowerCase().replace(" ", "-");
                  return (
                    <Link
                      key={subcategory.slug}
                      href={`/products/${categorySlug}/${subcategory.slug}`}
                      className="group relative bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:scale-105 border-2 border-purple-100 hover:border-purple-300"
                    >
                      {/* Image Container */}
                      ...
                      {/* Content */}
                      <div className="p-5">
                        <h4 className="text-lg font-bold text-gray-800 mb-2 group-hover:text-purple-600 transition-colors text-center">
                          {subcategory.name}
                        </h4>
                      ...
                      {/* Hover Indicator */}
                      <div className="absolute bottom-4 right-4 w-10 h-10 bg-purple-600 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity transform group-hover:scale-110">
                        <svg
